...
- **Designed a remote-only RAG layer** over the knowledge base, folded into the existing Worker request path with zero client-side round-trips or bundle bloat: chunks `app/knowledge.md` by `## App:` heading and cross-app pattern into 300–500-token slices, stores content in **D1** and embeddings in **Cloudflare Vectorize** (metadata-filtered by app inferred from the current screenshot's OCR / URL bar), retrieves top-K per turn server-side, then composes the final prompt before forwarding upstream. Replaces the current ~4 K-token verbatim `knowledge.md` injection with just the relevant chunks — sizeable per-turn token-cost reduction with no client-perceptible latency change. Explicitly rejected a local-first design (SQLite + `sqlite-vec` + on-device MiniLM ONNX) after judging that offline resilience adds no value when every turn already requires network for the vision call. Authoring flow: a `pnpm knowledge:push` script (Node/TS in `worker/`) diffs against D1, embeds changed chunks via a hosted embedding API, and atomically upserts D1 + Vectorize — ship a knowledge fix in seconds, no app rebuild.
...
- **Tracked the incremental impact of each grounding / reliability upgrade on an OSWorld task subset** (~40 tasks selected to match the production workload — file operations, browser navigation, image and code editors — run through the internal `run_and_review.py` harness, not submitted to the public leaderboard):

  | Pipeline state | Completion | Δ |
  |---|---|---|
  | Baseline (raw-pixel grounding, no SoM, no UIA) | ~20% | — |
  | + **Set-of-Mark** (RapidOCR text boxes + OpenCV icon contours) | ~35% | **+15 pp** |
  | + **Windows UIA merge** with the over-wide-element drop rule | ~50% | **+15 pp** |
  | + **Long-horizon reliability primitives** (in-run done-list, loop-break, fragmented MP4, close-before-launch, unconditional `MinimizeAll`) | ~58% | **+8 pp** |

  Landed at **~58%** — below the current OSWorld leaderboard frontier of **82–86%** (Alibaba's Qwen-agent stack on Qwen3.8-Max, Anthropic's built-in computer-use agent on Claude Fable 5), but comparable to mid-2025 frontier (Claude Sonnet 4.5 at 61.4%) as a solo agent-framework build. The gap to frontier is largely orchestration surface (planner / verifier / specialist grounders) that dedicated agent teams have layered on top — clear roadmap for phases 2–3 of the eval/RAG/multi-agent stack described elsewhere in the repo.
- On the tighter 5-task production-representative suite (`tools/baseline_tests.sh`), N ≥ 5 runs per task per config with Gemini 3.5-Flash: **`pass@1` ≈ 65–75%**, **`pass@3-of-5` ≈ 85–90%**. Higher than the OSWorld-subset number because the suite is narrower and heavily iterated on for the specific Chinese-language + web workflows we ship.
...
- **Nearly 3× improvement in OSWorld-subset completion rate** — from a raw-pixel baseline of ~20% to ~58% — attributed to specific engineering primitives (Set-of-Mark overlay, Windows UIA merge with over-wide-element drop rule, long-horizon reliability stack). Every landed change was ablated on the internal harness so the per-primitive impact is directly measurable, not a black-box "final number" claim.
