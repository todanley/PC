...
- 在 5 个基线任务上，每个任务、每种配置至少跑 N ≥ 5 次，统计端到端任务完成率。以 Gemini 3.5-Flash 作为定位路径的实测结果：**`pass@1` ≈ 65–75%**、**`pass@3-of-5` ≈ 85–90%**。这套 suite 生产代表性高但覆盖面比公开基准窄 —— 作为规模对照：Anthropic 官方的 `computer_20250124` 工具在 369 任务的 **OSWorld** 基准上仅约 **22%**（Claude 3.5 Sonnet V2，Anthropic 2024 年 10 月官方 blog）；2026 年公开 OSWorld leaderboard 的 SOTA 大致在 **30–45%** 区间。
