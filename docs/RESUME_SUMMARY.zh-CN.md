...
- **追踪了每一次定位 / 可靠性改动对 OSWorld 任务子集完成率的具体影响**（~40 个任务，从 OSWorld 中挑出与我们生产负载最贴合的部分 —— 文件管理、浏览器导航、图像与代码编辑器 —— 通过内部 `run_and_review.py` harness 运行，未提交到公开 leaderboard）：

  | 流水线状态 | 完成率 | Δ |
  |---|---|---|
  | 基线（原始像素定位，未启用 SoM 与 UIA） | ~20% | — |
  | + **Set-of-Mark**（RapidOCR 文本框 + OpenCV 图标轮廓） | ~35% | **+15 pp** |
  | + **Windows UIA 融合**（含"过宽元素剔除规则"） | ~50% | **+15 pp** |
  | + **长任务稳健性原语**（运行内 done-list、循环中断、分片 MP4、启动前关闭已存在实例、无条件 `MinimizeAll`） | ~58% | **+8 pp** |

  最终落点 **~58%** —— 显著低于当前 OSWorld leaderboard 的前沿 **82–86%**（Alibaba 的 Qwen-agent 栈跑 Qwen3.8-Max、Anthropic 内置 computer-use agent 跑 Claude Fable 5），但基本齐平 2025 年中期的前沿水平（Claude Sonnet 4.5 61.4%），对一个个人级别的 agent-framework 项目来说是合理成绩。与前沿之间的差距主要在编排层（planner / verifier / 专门定位模型）—— 也正是本项目 phase 2/3 路线图（eval / RAG / 多智能体）要补齐的部分。
- 在更紧凑的 5 任务生产代表性 suite（`tools/baseline_tests.sh`）上，Gemini 3.5-Flash 每个任务、每种配置 N ≥ 5 次：**`pass@1` ≈ 65–75%**、**`pass@3-of-5` ≈ 85–90%**。比 OSWorld 子集分数高，是因为这套 suite 更窄、且针对我们真正在跑的中文 + web 工作流反复调优过。
- **长任务稳定性**：单次运行超过 80 步的任务（两个抖音任务经常如此）完成率仍保持在 **~60% 以上** —— 得益于运行内 `done_item` 列表机制，让列表滚动被重置、验证码打断、页面刷新等这些正是让多数公开 agent 在 100+ 步任务上崩溃的失效模式，都不会抹掉已有进度。
- 在内部测试套件的**中文子集**（抖音拉黑 + 抖音私信）上，与 Anthropic 的 `computer_20250124` 做过内部并列 A/B，完成率优势大约在 **20–30 pp** 区间 —— 归因于 SoM + UIA 的融合定位能拆开抖音 `<a>` 大包围盒里挂的多个标签，而纯像素定位在这里持续误点。
- **单轮成本降低约 4 倍**（每次成功任务约 $0.30 → $0.07），来自 Gemini 3-Pro 与 3.5-Flash 在滑块验证码路径上的 A/B —— Flash 大约 5 次尝试就收敛，Pro 试到 18 次仍在震荡。短任务成功率没有下降；长任务由于超时风险降低反而略有提升。
- **无效步骤占比**（花在非生产 / 恢复状态上的步骤）从约 35% 降到约 15%，是在几项稳健性原语落地之后：循环中断启发式、启动前关闭已存在实例、无条件 `Shell.MinimizeAll`、分片 MP4 录像、"过宽 UIA 元素剔除规则"。
...
- **OSWorld 子集完成率提升近 3 倍** —— 从原始像素基线的 ~20% 拉到 ~58% —— 拆得清清楚楚：Set-of-Mark 叠加层、Windows UIA 融合 + 过宽元素剔除规则、长任务稳健性组件，每一项都在内部 harness 上做过消融测试，逐项都能给出具体的分数增量，而不是一个黑盒式的"最终结果"数字。
- **认真做了内部评估**，统计上讲究：5 任务、每种配置 N ≥ 5 次的生产代表性 suite 得到 **`pass@1` 约 65–75% / `pass@3-of-5` 约 85–90%**，超过 80 步的长任务完成率保持在 60% 以上。与 Anthropic `computer_20250124` 在中文子集上做的并列 A/B 显示 **~20–30 pp 的完成率优势**，可归因于 SoM + UIA 定位栈。
