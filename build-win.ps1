...
# Produces dist\UnboundComputerUse.exe (single self-contained onefile binary)
...
try {
    ...
    python -m PyInstaller unboundcomputeruse.spec --noconfirm --clean

    # Onefile build: single self-contained .exe in dist/.
    $exePath = "dist\UnboundComputerUse.exe"
