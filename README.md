...
app/
  ...
  som.py             — Set-of-Mark overlay (RapidOCR text + OpenCV icon contours)
...
- `turn_NN_marked.png` — SoM overlay (if enabled)
- `turn_NN.json` — model request/response (if `PHANTOM_TURN_DUMP=1`)
...
- **General patterns** — cross-app rules (launch via Spotlight, navigate back with mouse not `escape`, follow buttons encode current state not action, verify toggles landed, …).
...
Keep changes surgical, don't add abstractions for hypothetical future providers, and please run the app end-to-end on both macOS and Windows if you touch `platform_layer/`.
