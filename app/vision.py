...
- CLEAN-SLATE LAUNCH: when the task tells you to "open" or "use" an app and that app ALREADY has an open window (you can see its name in the taskbar / dock or its window peeking out from behind), CLOSE every existing instance FIRST, then launch it fresh. Without this, the agent inherits whatever screen the previous session left behind — a half-typed message, an open modal, a video detail page, a stale tab the model misreads as the home screen — and burns turns recovering instead of starting on the predictable landing surface. To close: focus the window then press `key: alt+f4`; if the app fights back with a "save changes?" dialog, click Discard / 不保存; repeat until no instance remains. THEN do the keyboard-launcher dance below to launch a fresh one.
- TO OPEN AN APP (browsers included): ALWAYS use the keyboard launcher — DO NOT click a taskbar / dock / desktop icon. Reason: similar-looking circular icons (Chrome vs Edge, Brave vs Vivaldi; iQiyi sitting next to Chrome) regularly fool the visual identifier and end up launching the wrong app, which then carries you down the wrong session / wrong logged-in state for the entire run. The keyboard launcher takes the app NAME — unambiguous. Sequence: (1) `key: win+r` on Windows / `key: {primary_mod}+space` on macOS to surface the launcher, (2) `type` the app's executable / native name — e.g. `chrome`, `msedge`, `notepad`, `计算器` (macOS Spotlight) — in its own script for non-Latin names, (3) `key: enter`. Skip this only when the requested app is already the foreground window (visible title bar matches).
- TO REACH A WEBSITE (gmail, douyin.com, x.com, github, anything on the web): once a browser window is open, click the address bar and `type` the site's BARE ROOT URL (e.g. `mail.google.com`, `douyin.com`, `x.com`), then press Enter. Do NOT click a bookmark, a favicon on the new-tab page, a recent-site tile, or a search result — those rely on small icons whose identity is easy to misread (an "Apple" favicon next to "Gmail" can fool any model after the screenshot is downscaled). Address-bar typing always lands on the right site regardless of what's pinned. Once the root page loads, every subsequent action uses the page UI — never type a sub-URL like `/user/self` or `/messages/inbox`; navigate by clicking visible affordances instead.{maximize_note}
